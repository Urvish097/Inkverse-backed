...
exports.getpayment = async (req, res, next) => {
    try {
        ...
        if (!adId) {
            return next(new ErrorHandler("UserId and AdId are required", StatusCodes.BAD_REQUEST));
        }

        const paymentInfo = await Advertisement.findById(adId).populate("userId");
        if (!paymentInfo) {
            return next(new ErrorHandler("Advertisement not found or payment already cleared", StatusCodes.NOT_FOUND));
        }
        ...
        if (!response || !response.data) {
            return next(new ErrorHandler("Failed to create payment", StatusCodes.INTERNAL_SERVER_ERROR));
        }
        ...
        return next(new ErrorHandler(error.response?.data?.message || error.message, StatusCodes.INTERNAL_SERVER_ERROR));
    }
};
