...
exports.updateBlog = async (req, res, next) => {
    try {
        ...
        // Update additional images if new ones are provided
        if (additionalimg.length > 0) { // Changed condition to check array length
            if (blog.additionalimg && blog.additionalimg.length > 0) { // Ensure there are existing additional images to delete
            ...
            const additional_images = await Promise.all(
                additionalimg.map(image => FilestorageFirebase.uploadToFierbase(image, userId, "userpost", "bloimages")) // Fixed typo
            );
